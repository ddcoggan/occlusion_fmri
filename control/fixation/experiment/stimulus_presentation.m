...
%%%% resolution
if debug == 1
	experiment.resolution = SetResolution(max(Screen('Screens')),1920,1080,60); % laptop
else
    experiment.resolution = SetResolution(max(Screen('Screens')),1920,1080,60); % scanner
end
...
params.screenWidth = 41;            % in cm; laptop=27.5, office=43, 3Tb=19, 7T=17, miniHelm=39;
params.viewingDist = 57;            % in cm; 3Tb/office=43, 7T=48, miniHelm=57;
...
if ~isempty(ET_ON)
    ...
    %el.backgroundcolour=[127 127 127];
...
if ~isempty(ET_ON)
    ...
    ETbox_rect = CenterRectOnPoint([0 0 params.ETbox_size_pix], xc, yc);
    ...
    while success == 0
        ...
        if ~isempty(params.ShowETbox)
            Screen('FrameOval', w, params.ETbox_color, ETbox_rect) ;%  [,penWidth]);
        end
        Screen('FrameOval', w, [0 0 0], params.Fixation.rect, params.Fixation.Width_pix)
        Screen('FillOval', w, [255 0 0], params.Fixation.rect2 ); %  , s.Fixation.Width_pix
        Screen('Flip',w);
...
if params.gammaCorrect > 0
    load('phase2_photometry_20151022_NewET.mat')
...
for b = 1:numel(conds)
    ...
    for t = 1:params.imsPerBlock
        ...
        GetEyeData
        gazesamples{b,t}.x = gazex(1:min([g_cnt max_ET_nframe]))';
        gazesamples{b,t}.y = gazey(1:min([g_cnt max_ET_nframe]))';
        gazesamples{b,t}.pa = gazepa(1:min([g_cnt max_ET_nframe]))';
        gazesamples{b,t}.TS = gazeTS(1:min([g_cnt max_ET_nframe]))';
        gazesamples{b,t}.D = gazeD(1:min([g_cnt max_ET_nframe]))';
    end
...
save(savename,'experiment','params','gazesamples');

KbQueueRelease();
ShowCursor;
Screen('Close');
Screen('CloseAll');
fprintf('Accuracy: (%.f)\nMean RT: (%f)\n', 100*experiment.task.accuracy, experiment.task.meanRT);

ecc = [];
for b = 1:length(conds)
    for t = 1:params.imsPerBlock
        x = gazesamples{b,t}.x-xc;
        y = gazesamples{b,t}.y-yc;
        eccPix = sqrt(x^2+y^2);
        eccDeg = eccPix/params.ppd;
        ecc = [ecc eccDeg];
    end
end
meanEcc = nanmean(ecc);
seEcc = nanstd( ecc ) / sqrt(sum(~isnan(ecc)));
fprintf('meanEcc: (%f)\nseEcc: (%f)', meanEcc, seEcc);
