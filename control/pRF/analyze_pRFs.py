...
def prf_linear_models():
    ...
    outpath = f'pRF_linear_models.csv'
...
def plot_prfs():
    ...
    plt.scatter(prfs['eccentricity'], prfs['fwhm'], alpha=.5,
                s=10, color='tab:blue', edgecolor='none', clip_on=False)
    print(f'mean V1 pRF size (FWHM): {prfs.fwhm.mean():.2f} dva')
    ...
    plt.savefig(f'../../figures/pRF_fits.pdf')
    plt.close()
